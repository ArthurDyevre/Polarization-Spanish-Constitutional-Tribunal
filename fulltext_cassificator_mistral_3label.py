...
OUTPUT_PATH = "tc_resolutions_fulltext_data_28May28_output_mistral_3label.csv"
VALIDATION_PATH = "random_sample_handcode_100.csv"

BATCH_SIZE  = 4  

MODEL_MAX_CONTEXT = 50_000
OUTPUT_RESERVED   = 150
PROMPT_OVERHEAD   = 200    
...
def init_model():
    ...
    llm = LLM(
        model=LOCAL_DIR,
        tokenizer_mode="mistral",     # Mistral-Small-3.1 uses the tekken tokenizer
        config_format="mistral",      # native Mistral config, not HF
        load_format="mistral",        # native Mistral weights
        dtype="auto",                 # resolves to bf16 for this checkpoint
        tensor_parallel_size=2,
        max_model_len=MODEL_MAX_CONTEXT,
        gpu_memory_utilization=0.85,  # leave headroom; default 0.9 can OOM on long KV caches
        # This is a vision model; we only send text, so don't reserve image slots.
        limit_mm_per_prompt={"image": 0},
        # vLLM v1 enables prefix caching by default. With TP=2 + CUDA graphs this
        # triggers an illegal memory access when cached KV blocks from a previous
        # batch are reused (num_common_prefix_blocks > 0). Disable until fixed upstream.
...
def count_tokens(text: str) -> int:
    """Return an approximate token count for *text* using the engine's tokenizer.

    Used only for length budgeting / truncation, so exactness is not critical.
    The Mistral tokenizer's encode() takes no HF-style kwargs, hence the bare call.
    """
    if not text:
        return 0
...
def select_text(full_text: str, tipo_resolucion: str) -> tuple[str, str]:
    ...
    truncated = tokenizer.decode(ids[:available]) if available > 0 else ""
...
def build_messages(text: str, source_label: str, system_prompt: str, tipo_resolucion: str) -> list[dict]:
    """
    Build the chat message list for llm.chat(). vLLM applies the Mistral chat
    template internally, so we just return role/content dicts — no manual
    templating, no reasoning kwargs (Mistral-Small is not a reasoning model).

    select_text() has already truncated the body to fit _DOC_BUDGET, which
    reserves room for the system prompt, output, and template overhead, so no
...
def run_batch(conversations: list[list[dict]]) -> list[str]:
...
def main():
    ...
    for i in range(0, len(df), BATCH_SIZE):
        ...
        # ── Pass 1: territorial ───────────────────────────────────────────
        terr_convs = [
            build_messages(text, label, TERRITORIAL_SYSTEM, row.tipo_resolucion)
            for text, label, row in zip(texts, source_labels, batch.itertuples())
        ]
        terr_raw = run_batch(terr_convs)

        # ── Pass 2: politization ──────────────────────────────────────────
        pol_convs = [
            build_messages(text, label, POLITIZATION_SYSTEM, row.tipo_resolucion)
            for text, label, row in zip(texts, source_labels, batch.itertuples())
        ]
        pol_raw = run_batch(pol_convs)
